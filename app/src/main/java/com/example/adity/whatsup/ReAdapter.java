...
public class ReAdapter extends RecyclerView.Adapter<ReAdapter.MyViewHolder>{
    ...
    public ReAdapter(Context context, List<Item> data,int store){
    ...
    public ReAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if(Store==1) {
            ...
            View view = inflater.inflate(R.layout.custom_row_call, parent, false);
            MyViewHolder holder=new MyViewHolder(view);
            return holder;
        }
    }
    ...
    class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            if(mContext instanceof MainActivity ){
                if(Store==1){
                    ((MainActivity)mContext).chats();
                }
                else if(Store==3){
                    ((MainActivity)mContext).callscreen();
                }
